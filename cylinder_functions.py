...
class BaseState:
    ...
    def __init__(
        ...
        self.gr_interp = interpolate.interp1d(
            self.R, self.gr, kind="cubic", bounds_error=False, fill_value="extrapolate"
        )

        # Interpolate gt
        self.gt_interp = interpolate.interp1d(
            self.R, self.gt, kind="cubic", bounds_error=False, fill_value="extrapolate"
        )
    ...
    def entropy(state1, state2, Rs, dt):
        ...
        strain_energy_1 = np.array(
            [state1.strain_energy_density(state1.find_inner_radius(), R) for R in Rs]
        )
        strain_energy_2 = np.array(
            [state2.strain_energy_density(state2.find_inner_radius(), R) for R in Rs]
        )
    ...
    def power_direct(state1, state2, ss, dt):
        ...
        F1_rr = [
            (s / state1.compute_r(ri1, s)) * state1.gt_interp(s) * state1.gr_interp(s)
            for s in ss
        ]
        ...
        F2_rr = [
            (s / state2.compute_r(ri2, s)) * state2.gt_interp(s) * state2.gr_interp(s)
            for s in ss
        ]
        ...
        # integrate power
        int_power = (
            2 * np.pi * np.trapezoid(power * ss, ss)
        )  # multiply by R for volume element in cylindrical coordinates
    ...
    def Mandel_stress(self, ri, s):
        ...
        M_rr = self.mu * (s / r_val) ** 2 * (gt_val / gr_val) + p_val / (
            gr_val * gt_val
        )

        # Circumferential Mandel stress
        M_tt = self.mu * (r_val / s) ** 2 / (gr_val * gt_val**3) + p_val / (
            gr_val * gt_val
        )
    ...
    def find_inner_radius(self):
        ...
        try:
            return brentq(objective, 0.1, 3, xtol=1e-6, maxiter=20)
        except:
    ...
    def Ricci_curvature(self, ri, s):
        ...
        if s == self._Ri:  # Start boundary: use forward differences
            ...
            gr_R = (
                -3 * self.gr_interp(s)
                + 4 * self.gr_interp(s + h)
                - self.gr_interp(s + 2 * h)
            ) / (2 * h)
            gt_R = (
                -3 * self.gt_interp(s)
                + 4 * self.gt_interp(s + h)
                - self.gt_interp(s + 2 * h)
            ) / (2 * h)
            # Second derivative (1st order forward difference)
            gt_RR = (
                self.gt_interp(s + 2 * h)
                - 2 * self.gt_interp(s + h)
                + self.gt_interp(s)
            ) / (h**2)
    ...
    def update(self):
        ...
        return self.__class__(
            ...
            viscosity_const=self.viscosity_const,
    ...
    def __str__(self):
        return (
            f"State(_Ri={self._Ri}, gr={self.gr}, "
