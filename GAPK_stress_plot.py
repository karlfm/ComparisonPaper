...
def plot_results(file_name):
    ...
    plotter_instance = plotter.ComparisonPlotter(
        R_range, number_of_lines, model_name="GAPK (stress based)"
    )

    # Plot spatial data
    plotter_instance.plot_spatial_panel(
        (0, 0),
        "Radial Stress (Cauchy)",
        "Stress",
        plot_data["radial_stress"],
        set_point=set_point,
    )
    plotter_instance.plot_spatial_panel(
        (1, 0),
        "Hoop Stress (Cauchy)",
        "Stress",
        plot_data["hoop_stress"],
        set_point=set_point,
    )
    plotter_instance.plot_spatial_panel(
        (0, 1), "Radial Strain", "Strain", plot_data["radial_strain"]
    )
    plotter_instance.plot_spatial_panel(
        (1, 1), "Hoop Strain", "Strain", plot_data["hoop_strain"]
    )
    plotter_instance.plot_spatial_panel(
        (0, 2), "Radial Growth", "Growth", plot_data["radial_growth"], set_point=gMax
    )
    plotter_instance.plot_spatial_panel(
    ...
    stimulus_plotter = plotter.ComparisonPlotter(
        R_range, number_of_lines, model_name="GAPK (stress based)", grid_shape=(1, 1)
    )
