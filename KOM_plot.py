...
def plot_results(file_name):
    ...
    plotter_instance.plot_spatial_panel(
        (0, 0), "Radial Stress (Cauchy)", "Stress", plot_data["radial_stress"]
    )
    plotter_instance.plot_spatial_panel(
        (1, 0), "Hoop Stress (Cauchy)", "Stress", plot_data["hoop_stress"]
    )
    plotter_instance.plot_spatial_panel(
        (0, 1),
        "Radial Strain",
    ...
    stimulus_plotter = plotter.ComparisonPlotter(
        R_range, num_steps, model_name="KOM", grid_shape=(2, 1)
    )

    stimulus_plotter.plot_spatial_panel(
        (0, 0), "Radial Stimulus (sr)", "Stimulus", plot_data["sr"]
    )
    stimulus_plotter.plot_spatial_panel(
        (1, 0), "Hoop Stimulus (sf)", "Stimulus", plot_data["sf"]
    )
    ...
    time_labels = [
        f"t = {int(i * num_steps / (number_of_lines - 1)) * dt:.0f}"
        for i in range(number_of_lines)
    ]

    # Growth term (limits based on current gt)
    ax = axes_hoop[0, 0]
    for i, (data_line, color, label) in enumerate(
        zip(plot_data["growth_term_gt"], colors, time_labels)
    ):
    ...
    ax = axes_hoop[0, 1]
    for i, (data_line, color, label) in enumerate(
        zip(plot_data["strain_term_gt"], colors, time_labels)
    ):
        ax.plot(R_range, data_line, color=color, label=label)
    ax.set_xlabel("R (Reference)")
    ...
    ax = axes_hoop[1, 0]
    for i, (data_line, color, label) in enumerate(
        zip(plot_data["F_inc_t"], colors, time_labels)
    ):
        ax.plot(R_range, data_line, color=color, label=label)
    ax.set_xlabel("R (Reference)")
    ax.set_ylabel("F_inc_t")
    ax.set_title(
        r"Growth Increment: $F_{inc,\theta} = \tau \cdot growth \cdot strain + 1$"
    )
    ...
    ax = axes_hoop[1, 1]
    for i, (data_line, color, label) in enumerate(
        zip(plot_data["hoop_growth"], colors, time_labels)
    ):
        ax.plot(R_range, data_line, color=color, label=label)
    ax.set_xlabel("R (Reference)")
    ...
    fig_rad.suptitle(
        "KOM Model - Radial (Cross-Fiber) Growth Sigmoid Components", fontsize=18
    )

    # Growth term (limits based on current gr)
    ax = axes_rad[0, 0]
    for i, (data_line, color, label) in enumerate(
        zip(plot_data["growth_term_gr"], colors, time_labels)
    ):
        ax.plot(R_range, data_line, color=color, label=label)
    ax.set_xlabel("R (Reference)")
    ax.set_ylabel("Growth Term")
    ...
    ax = axes_rad[0, 1]
    for i, (data_line, color, label) in enumerate(
        zip(plot_data["strain_term_gr"], colors, time_labels)
    ):
        ax.plot(R_range, data_line, color=color, label=label)
    ax.set_xlabel("R (Reference)")
    ax.set_ylabel("Strain Term")
    ...
    ax = axes_rad[1, 0]
    for i, (data_line, color, label) in enumerate(
        zip(plot_data["F_inc_r"], colors, time_labels)
    ):
        ax.plot(R_range, data_line, color=color, label=label)
    ax.set_xlabel("R (Reference)")
    ax.set_ylabel("F_inc_r")
    ax.set_title(
        r"Growth Increment: $F_{inc,r} = \sqrt{\tau \cdot growth \cdot strain + 1}$"
    )
    ax.axhline(y=1.0, color="k", linestyle="--", alpha=0.5, label="No growth")
    ax.legend(fontsize=10)
    ax.grid(True, alpha=0.4)
    ...
    ax = axes_rad[1, 1]
    for i, (data_line, color, label) in enumerate(
        zip(plot_data["radial_growth"], colors, time_labels)
    ):
        ax.plot(R_range, data_line, color=color, label=label)
    ax.set_xlabel("R (Reference)")
    ax.set_ylabel(r"$g_r$")
    ...
    print(
        "Saved: ODE_KOM_results.png, ODE_KOM_stimulus.png, ODE_KOM_hoop_sigmoids.png, ODE_KOM_radial_sigmoids.png"
    )
